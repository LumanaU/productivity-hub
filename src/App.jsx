        ...
        if (missing.length > 0) {
          ...
          if (refreshed) {
            ...
            const analystMap = refreshed.map(a => {
              ...
              return { ...a, enterpriseId: a.enterprise_id, fullName: a.full_name, active: a.active !== false, role: prof?.role || "analyst", marks: marksByDate };
            });
      ...
      <div style={s.section}>
        ...
        {activeAnalysts.length===0?(
          ...
          {analystUsers.length===0?(
            <div style={{...s.card,textAlign:"center",padding:"24px",color:txt2,fontSize:"13px"}}>No analysts yet — they'll appear when they create accounts.</div>
          ):analystUsers.map(analyst=>{
            const dayMarks = analyst.marks?.[adminDate] || {};
            const duration = calcAnalystDuration(dayMarks, true);
            const isAct = dayMarks.clock_in && !dayMarks.clock_out;
            const online = isAnalystOnline(analyst);
        ...
        <div style={s.card}>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
            <button onClick={()=>loadAnalystsFromDb()} style={s.btn(acc)}>🔄 Refresh data now</button>
