...
export default function App({ authUser, profile, onLogout }) {
  // ── Auth derived from props ──────────────────────────────
  ...
  const [newAnalyst,setNewAnalyst]       = useState({enterpriseId:"",fullName:"",email:""});
  ...
  const loadAnalystsFromDb = useCallback(async () => {
    ...
    try {
      // Load profiles to know roles + catch anyone missing from analysts
      const { data: profilesData } = await supabase.from("profiles").select("*");
      const { data } = await supabase.from("analysts").select("*").order("created_at");
      
      // Auto-create analyst records for any profiles missing from analysts table
      if (profilesData && data) {
        const existingEmails = new Set((data||[]).map(a=>(a.email||"").toLowerCase()).filter(Boolean));
        const missing = profilesData.filter(p => p.email && !existingEmails.has(p.email.toLowerCase()));
        for (const p of missing) {
          await supabase.from("analysts").insert({
            enterprise_id: p.email.split("@")[0],
            full_name: p.full_name || p.email.split("@")[0],
            email: p.email.toLowerCase(),
            active: true,
          }).select();
        }
        // Reload if we added anyone
        if (missing.length > 0) {
          const { data: refreshed } = await supabase.from("analysts").select("*").order("created_at");
          if (refreshed) {
            const { data: marksData } = await supabase.from("marks").select("*").order("marked_at");
            const analystMap = refreshed.map(a => {
              const aMarks = (marksData || []).filter(m => m.analyst_id === a.id);
              const marksByDate = {};
              aMarks.forEach(m => { if (!marksByDate[m.mark_date]) marksByDate[m.mark_date] = {}; marksByDate[m.mark_date][m.mark_type] = m.marked_at; });
              const prof = profilesData.find(p => p.email?.toLowerCase() === a.email?.toLowerCase());
              return { ...a, enterpriseId: a.enterprise_id, fullName: a.full_name, active: a.active !== false, role: prof?.role || "analyst", teamId: a.team_id || null, marks: marksByDate };
            });
  ...
  useEffect(() => {
    ...
    const dataChannel = supabase.channel("realtime-marks")
    ...
    const presenceChannel = supabase.channel("online-users", { config: { presence: { key: authUser?.id || "anon" } } });
    presenceChannel
      .on("presence", { event: "sync" }, () => {
        const state = presenceChannel.presenceState();
        const online = {};
        Object.values(state).forEach(users => {
          users.forEach(u => { if(u.email) online[u.email.toLowerCase()] = { since: u.online_at, userId: u.user_id }; });
        });
  ...
  }, [loadAnalystsFromDb, authUser]);

  // Helper: check if an analyst is online
  const isAnalystOnline = (analyst) => {
    const email = (analyst.email || "").toLowerCase();
    const entId = (analyst.enterpriseId || "").toLowerCase();
    return Object.keys(onlineUsers).some(ue =>
      (email && ue === email) || (entId && ue.startsWith(entId)) || (entId && ue.split("@")[0] === entId)
    );
  };

  // ── Supabase: Auto-link current user to their analyst record ─
  const [currentAnalystId,setCurrentAnalystId] = useState("");
  const autoLinkRetried = useRef(false);
  useEffect(() => {
    if (!authUser || !analysts.length) {
      // If no analysts loaded yet, try loading after a short delay (SQL trigger may be creating the record)
      if (authUser && !autoLinkRetried.current) {
        autoLinkRetried.current = true;
        setTimeout(() => loadAnalystsFromDb(), 2000);
      }
      return;
    }
    ...
    const match = analysts.find(a => {
      ...
      const aEntId = (a.enterpriseId || "").toLowerCase();
      return (aEmail && aEmail === userEmail) ||
             (aEntId && userEmail.startsWith(aEntId)) ||
             (aEntId && userEmail.split("@")[0] === aEntId);
    });
    if (match) setCurrentAnalystId(match.id);
    else if (isAdmin) setCurrentAnalystId("");
    else if (!autoLinkRetried.current) {
      autoLinkRetried.current = true;
      setTimeout(() => loadAnalystsFromDb(), 2000);
    }
  ...
  const addAnalyst = async () => {
    const {enterpriseId,fullName,email} = newAnalyst;
    ...
    if (supabaseReady && supabase) {
      ...
      if (error) { setPopup({title:"❌ Add failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      await loadAnalystsFromDb();
    } else {
      setAnalysts(prev=>[...prev,{
        id: Date.now().toString(), enterpriseId: enterpriseId.trim(),
        fullName: fullName.trim() || enterpriseId.trim(), email: email.trim().toLowerCase(), marks: {},
      }]);
    }
    setNewAnalyst({enterpriseId:"",fullName:"",email:""});
  };

  const deactivateAnalyst = async (id) => {
    const target = analysts.find(a=>a.id===id);
    if (supabaseReady && supabase) {
      const { error } = await supabase.from("analysts").update({ active: false }).eq("id", id);
      if (error) { setPopup({title:"❌ Deactivate failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      await loadAnalystsFromDb();
    } else {
      setAnalysts(prev=>prev.map(a=>a.id===id?{...a,active:false}:a));
    }
    if(currentAnalystId===id) setCurrentAnalystId("");
    if(target){
      setPopup({title:`⏸ ${target.fullName} deactivated`, body:"Analyst removed from active roster. Can be reactivated anytime."});
      setTimeout(()=>setPopup(null),3500);
    }
  };

  const reactivateAnalyst = async (id) => {
    const target = analysts.find(a=>a.id===id);
    if (supabaseReady && supabase) {
      await supabase.from("analysts").update({ active: true }).eq("id", id);
      loadAnalystsFromDb();
    } else {
      setAnalysts(prev=>prev.map(a=>a.id===id?{...a,active:true}:a));
    }
    if(target){
      setPopup({title:`✅ ${target.fullName} reactivated`, body:"Analyst is back on the active roster."});
      setTimeout(()=>setPopup(null),3500);
    }
  };

  // Split analysts into active/inactive and admin/analyst
  const activeAnalysts = analysts.filter(a=>a.active !== false);
  const inactiveAnalysts = analysts.filter(a=>a.active === false);
  const adminUsers = activeAnalysts.filter(a=>a.role === "admin");
  const analystUsers = activeAnalysts.filter(a=>a.role !== "admin");

  // Keep old function name for backward compat but just deactivate
  const removeAnalyst = deactivateAnalyst;

  // ── Team management ──────────────────────────────────────
  const createTeam = async () => {
    if (!newTeamName.trim()) return;
    if (supabaseReady && supabase) {
      const { error } = await supabase.from("teams").insert({ name: newTeamName.trim(), created_by: authUser?.id });
      if (error) { setPopup({title:"❌ Create team failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      setPopup({title:"✅ Team created",body:`"${newTeamName.trim()}" is ready.`}); setTimeout(()=>setPopup(null),3000);
      await loadAnalystsFromDb();
    }
    setNewTeamName("");
  };
  ...
  const deleteTeam = async (teamId) => {
    if (supabaseReady && supabase) {
      await supabase.from("analysts").update({ team_id: null }).eq("team_id", teamId);
      const { error } = await supabase.from("teams").delete().eq("id", teamId);
      if (error) { setPopup({title:"❌ Delete failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      await loadAnalystsFromDb();
    }
    setPopup({title:"🗑 Team deleted",body:"Analysts moved to unassigned."});
    setTimeout(()=>setPopup(null),3000);
  };

  const assignToTeam = async (analystId, teamId) => {
    if (supabaseReady && supabase) {
      const { error } = await supabase.from("analysts").update({ team_id: teamId || null }).eq("id", analystId);
      if (error) { setPopup({title:"❌ Assign failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      await loadAnalystsFromDb();
    }
  };

  const setTeamLeader = async (teamId, leaderId) => {
    if (supabaseReady && supabase) {
      const { error } = await supabase.from("teams").update({ leader_id: leaderId || null }).eq("id", teamId);
      if (error) { setPopup({title:"❌ Set leader failed",body:error.message}); setTimeout(()=>setPopup(null),5000); return; }
      await loadAnalystsFromDb();
    }
  };

  // Filter analysts by team
  const getFilteredAnalysts = (list) => {
    if (teamFilter === "all") return list;
    if (teamFilter === "unassigned") return list.filter(a => !a.teamId);
    return list.filter(a => a.teamId === teamFilter);
  };

  const filteredAnalystUsers = getFilteredAnalysts(analystUsers);
  const getTeamName = (teamId) => teams.find(t=>t.id===teamId)?.name || "Unassigned";
  const getTeamLeader = (teamId) => { const t=teams.find(x=>x.id===teamId); return t?.leader_id ? analysts.find(a=>a.id===t.leader_id) : null; };

  // ── History date range helpers ────────────────────────────
  const getHistoryDates = () => {
    const d = new Date(historyDate);
    if (historyRange === "day") return [historyDate, historyDate];
  ...
  const getHistoryMarks = () => {
    const [startDate, endDate] = getHistoryDates();
    const source = historyAnalyst === "all" ? analysts : analysts.filter(a => a.id === historyAnalyst);
    const rows = [];
    source.forEach(a => {
      if (!a.marks) return;
      Object.keys(a.marks).sort().forEach(date => {
        if (date >= startDate && date <= endDate) {
          const hasAny = MARK_KEYS.some(k => a.marks[date]?.[k.key]);
          if (hasAny) rows.push({ analyst: a, date, marks: a.marks[date] });
        }
      });
    });
    return rows;
  ...
  const exportHistoryCSV = () => {
    const rows = getHistoryMarks();
    if (!rows.length) { setPopup({title:"⚠ No data",body:"No marks found for this period."}); setTimeout(()=>setPopup(null),3000); return; }
    const header = ["Enterprise ID","Full Name","Date",...MARK_KEYS.map(k=>k.label),"Worked Time (HH:MM:SS)"];
    const csvRows = rows.map(r => [
      `"${r.analyst.enterpriseId}"`, `"${r.analyst.fullName}"`, r.date,
      ...MARK_KEYS.map(k => r.marks[k.key] ? `"${fmtTimeOnly(r.marks[k.key])}"` : ""),
      calcAnalystDuration(r.marks, true) || "",
    ].join(","));
    const [s,e] = getHistoryDates();
    safeDownload(header.join(",")+"\n"+csvRows.join("\n"), `history-${s}-to-${e}.csv`);
  };
  ...
  const BottomNav = (
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:9990,background:card,borderTop:`1px solid ${bdr}`,
      ...
      {isAdminView&&<button onClick={()=>{setShowAdminPanel(true);setShowEnterprise(false);loadAnalystsFromDb();}}
        style={{flex:1,maxWidth:"140px",background:showAdminPanel?"#ef444418":"transparent",border:`1.5px solid ${showAdminPanel?"#ef4444":bdr}`,
          borderRadius:"10px",padding:"8px 6px",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
        <div style={{fontSize:"16px"}}>👑</div>
        <div style={{fontSize:"10px",fontWeight:600,color:showAdminPanel?"#ef4444":txt2,marginTop:"2px"}}>Admin</div>
      </button>}
    </div>
  );

  // Block non-admins from admin panel
  useEffect(() => { if(showAdminPanel && !isAdminView) setShowAdminPanel(false); }, [showAdminPanel, isAdminView]);

  if(showAdminPanel && isAdminView) return (
    <div style={s.wrap}>
      ...
      <div style={s.section}>
        ...
        {isAdminView&&<div style={s.card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:"8px",alignItems:"end"}}>
            ...
            <div>
              <label style={s.lbl}>Email (for auto-linking)</label>
              <input placeholder="jane.smith@company.com" value={newAnalyst.email}
                onChange={e=>setNewAnalyst(p=>({...p,email:e.target.value}))}
                onKeyDown={e=>e.key==="Enter"&&addAnalyst()}
                style={s.input} />
            </div>
          ...
          <div style={{fontSize:"11px",color:txt2,marginTop:"8px",lineHeight:1.5}}>
            💡 <strong>Auto-linking:</strong> Enter the same email the analyst will use to create their account. They'll be automatically matched to their roster record — no manual selection needed.
            <br/>📥 <strong>Bulk import:</strong> Download the template, fill it with your roster, then upload via Import CSV.
          </div>
        </div>}
        ...
        {isAdminView&&analysts.length>0 && (
          <div style={s.card}>
            <div style={{fontWeight:600,fontSize:"15px",marginBottom:"10px",color:txt}}>👥 Manage Team</div>
            <div style={{display:"flex",gap:"8px",alignItems:"stretch",flexWrap:"wrap",marginBottom:"10px"}}>
              <select value={analystToDelete} onChange={e=>setAnalystToDelete(e.target.value)}
                ...
                <optgroup label="Active">
                  {activeAnalysts.map(a=><option key={a.id} value={a.id}>{a.fullName}{a.email?` · ${a.email}`:""}</option>)}
                </optgroup>
                {inactiveAnalysts.length>0&&<optgroup label="Inactive">
                  {inactiveAnalysts.map(a=><option key={a.id} value={a.id}>⏸ {a.fullName}{a.email?` · ${a.email}`:""}</option>)}
                </optgroup>}
              </select>
              <button onClick={()=>{
                if(!analystToDelete) return;
                const t=analysts.find(x=>x.id===analystToDelete);
                if(!t) return;
                if(t.active!==false) deactivateAnalyst(analystToDelete);
                else reactivateAnalyst(analystToDelete);
                setAnalystToDelete("");
              }} style={s.btn(analysts.find(x=>x.id===analystToDelete)?.active===false?"#10b981":"#f59e0b")}>
                {analysts.find(x=>x.id===analystToDelete)?.active===false?"✅ Reactivate":"⏸ Deactivate"}
              </button>
            </div>
            {inactiveAnalysts.length>0&&(
              <div style={{fontSize:"12px",color:txt2}}>
                ⏸ {inactiveAnalysts.length} inactive analyst{inactiveAnalysts.length>1?"s":""}
                <button onClick={()=>setShowInactive(!showInactive)} style={{background:"none",border:"none",color:acc,cursor:"pointer",fontWeight:600,fontSize:"12px",marginLeft:"6px"}}>{showInactive?"Hide":"Show"}</button>
              </div>
            )}
            {showInactive&&inactiveAnalysts.map(a=>(
              <div key={a.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${bdr}`,opacity:0.6}}>
                <div><span style={{fontSize:"14px"}}>⏸</span> <span style={{fontSize:"13px",color:txt2}}>{a.fullName}</span> <span style={{fontSize:"11px",color:txt2}}>{a.email||""}</span></div>
                <button onClick={()=>reactivateAnalyst(a.id)} style={{...s.btn("#10b981",true),padding:"3px 10px",fontSize:"11px"}}>✅ Reactivate</button>
              </div>
            ))}
          </div>
        )}
        ...
        {isAdminView&&analysts.length>0&&(
          <div style={s.card}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px",flexWrap:"wrap",gap:"8px"}}>
              <div style={{fontWeight:600,fontSize:"15px",color:txt}}>📊 Mark History</div>
              <button onClick={()=>setShowHistory(!showHistory)} style={s.btn(acc,!showHistory)}>
            ...
            {showHistory&&(
              <>
                <div style={{display:"flex",gap:"10px",flexWrap:"wrap",marginBottom:"14px",alignItems:"flex-end"}}>
                  <div>
                    <label style={s.lbl}>Range</label>
                    <select value={historyRange} onChange={e=>setHistoryRange(e.target.value)} style={{...s.input,width:"120px",fontSize:"13px"}}>
                  ...
                  <div>
                    <label style={s.lbl}>{historyRange==="day"?"Date":historyRange==="week"?"Week of":"Month of"}</label>
                    <input type="date" value={historyDate} onChange={e=>setHistoryDate(e.target.value)} style={{...s.input,width:"160px",fontSize:"13px"}} />
                  </div>
                  <div>
                    <label style={s.lbl}>Analyst</label>
                    <select value={historyAnalyst} onChange={e=>setHistoryAnalyst(e.target.value)} style={{...s.input,width:"180px",fontSize:"13px"}}>
                      <option value="all">All analysts</option>
                      {analysts.map(a=><option key={a.id} value={a.id}>{a.fullName}</option>)}
                    </select>
                  </div>
                  <button onClick={exportHistoryCSV} style={s.btn("#10b981")}>⬇ Export</button>
                </div>
                <div style={{fontSize:"12px",color:txt2,marginBottom:"10px"}}>
                  Showing {getHistoryDates()[0]} to {getHistoryDates()[1]} · {getHistoryMarks().length} record{getHistoryMarks().length!==1?"s":""}
                </div>
                <div style={{maxHeight:"400px",overflowY:"auto"}}>
                  {getHistoryMarks().length===0?(
                    <div style={{color:txt2,textAlign:"center",padding:"24px",fontSize:"13px"}}>No marks found for this period.</div>
                  ):getHistoryMarks().map((r,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 0",borderBottom:`1px solid ${bdr}`,flexWrap:"wrap"}}>
                      <div style={{minWidth:"100px",fontSize:"12px",fontWeight:600,color:txt}}>{r.analyst.fullName}</div>
                      <div style={{fontSize:"11px",color:txt2,minWidth:"80px"}}>{r.date}</div>
                      <div style={{display:"flex",gap:"4px",flex:1,flexWrap:"wrap"}}>
                        {MARK_KEYS.map(k=>r.marks[k.key]?(
                          <span key={k.key} style={{background:k.color+"18",color:k.color,borderRadius:"6px",padding:"2px 6px",fontSize:"10px",fontWeight:600}}>
                            {k.icon} {fmtTimeOnly(r.marks[k.key])}
                          </span>
                        ):null)}
                      </div>
                      <div style={{fontSize:"11px",fontWeight:600,color:acc,fontVariantNumeric:"tabular-nums"}}>{calcAnalystDuration(r.marks,false)||"—"}</div>
                    </div>
                  ))}
                </div>
              </>
        ...
        {isAdminView&&(
          <div style={s.card}>
            <div style={{fontWeight:600,fontSize:"15px",marginBottom:"12px",color:txt}}>🏷 Teams</div>
            <div style={{display:"flex",gap:"8px",marginBottom:"12px",flexWrap:"wrap"}}>
              <input placeholder="New team name..." value={newTeamName}
                onChange={e=>setNewTeamName(e.target.value)}
                onKeyDown={e=>e.key==="Enter"&&createTeam()}
                style={{...s.input,flex:1,minWidth:"160px",fontSize:"13px"}} />
              <button onClick={createTeam} style={s.btn(acc)}>+ Create team</button>
            </div>
            {teams.length===0?(
              <div style={{color:txt2,fontSize:"13px",textAlign:"center",padding:"16px"}}>No teams yet. Create one above.</div>
            ):teams.map(team=>{
              const leader = team.leader_id ? analysts.find(a=>a.id===team.leader_id) : null;
              const members = analysts.filter(a=>a.teamId===team.id && a.active!==false);
              return (
                <div key={team.id} style={{border:`1.5px solid ${bdr}`,borderRadius:lr,padding:"14px",marginBottom:"10px"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px",flexWrap:"wrap",gap:"6px"}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:"14px",color:txt}}>{team.name}</div>
                      <div style={{fontSize:"11px",color:txt2}}>{members.length} member{members.length!==1?"s":""} · Leader: {leader?leader.fullName:"None assigned"}</div>
                    </div>
                    <button onClick={()=>safeConfirm("Delete team?",`"${team.name}" will be deleted. Members move to unassigned.`,()=>deleteTeam(team.id))}
                      style={{...s.btn("#ef4444",true),padding:"3px 10px",fontSize:"11px"}}>Delete</button>
                  </div>
                  <div style={{display:"flex",gap:"8px",marginBottom:"8px",flexWrap:"wrap",alignItems:"flex-end"}}>
                    <div style={{flex:1,minWidth:"160px"}}>
                      <label style={s.lbl}>Team Leader (Admin)</label>
                      <select value={team.leader_id||""} onChange={e=>setTeamLeader(team.id,e.target.value||null)}
                        style={{...s.input,fontSize:"13px"}}>
                        <option value="">— Select admin —</option>
                        {adminUsers.map(a=><option key={a.id} value={a.id}>👑 {a.fullName}</option>)}
                      </select>
                    </div>
                    <div style={{flex:1,minWidth:"160px"}}>
                      <label style={s.lbl}>Add member</label>
                      <select value="" onChange={e=>{if(e.target.value)assignToTeam(e.target.value,team.id);}}
                        style={{...s.input,fontSize:"13px"}}>
                        <option value="">— Add analyst —</option>
                        {activeAnalysts.filter(a=>a.teamId!==team.id && a.role!=="admin").map(a=>
                          <option key={a.id} value={a.id}>{a.fullName}{a.teamId?` (in ${getTeamName(a.teamId)})`:""}</option>
                        )}
                      </select>
                    </div>
                  </div>
                  {members.length>0&&(
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
                      {members.map(m=>(
                        <span key={m.id} style={{display:"inline-flex",alignItems:"center",gap:"6px",background:m.id===team.leader_id?acc+"18":"#f3f4f6",border:`1px solid ${m.id===team.leader_id?acc+"44":bdr}`,borderRadius:"8px",padding:"4px 10px",fontSize:"12px",color:txt}}>
                          {m.id===team.leader_id?"👑":""} {m.fullName}
                          <button onClick={()=>assignToTeam(m.id,null)} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:"14px",padding:0}}>×</button>
                        </span>
                      ))}
                    </div>
                  )}
        ...
        {/* ── Team filter for analyst view ──────────── */}
        {activeAnalysts.length>0&&teams.length>0&&(
          <div style={{display:"flex",gap:"8px",marginBottom:"6px",flexWrap:"wrap",alignItems:"center"}}>
            <span style={{fontSize:"12px",fontWeight:600,color:txt2}}>Filter:</span>
            <button onClick={()=>setTeamFilter("all")} style={s.tabBtn(teamFilter==="all")}>All</button>
            <button onClick={()=>setTeamFilter("unassigned")} style={s.tabBtn(teamFilter==="unassigned")}>Unassigned</button>
            {teams.map(t=><button key={t.id} onClick={()=>setTeamFilter(t.id)} style={s.tabBtn(teamFilter===t.id)}>{t.name}</button>)}
          </div>
        )}
        {activeAnalysts.length>0&&(()=>{
          const today = adminDate;
          const onlineCount = activeAnalysts.filter(a=>isAnalystOnline(a)).length;
          const clockedIn  = activeAnalysts.filter(a=>a.marks?.[today]?.clock_in && !a.marks?.[today]?.clock_out).length;
          const onBreak    = activeAnalysts.filter(a=>{
            const m=a.marks?.[today]; if(!m) return false;
            return (m.break_in_1&&!m.break_out_1) || (m.break_in_2&&!m.break_out_2);
          }).length;
          const onLunch    = activeAnalysts.filter(a=>a.marks?.[today]?.lunch_in && !a.marks?.[today]?.lunch_out).length;
          const completed  = activeAnalysts.filter(a=>a.marks?.[today]?.clock_out).length;
          return (
            <div style={{...s.card,display:"flex",gap:"12px",flexWrap:"wrap",justifyContent:"space-around"}}>
              {[
                ["👥","Total",activeAnalysts.length,acc],
                ["🌐","Online",onlineCount,"#10b981"],
                ["🟢","Clocked In",clockedIn,"#059669"],
        ...
        {/* Render analyst card helper */}
        {activeAnalysts.length===0?(
          <div style={{...s.card,textAlign:"center",padding:"40px 20px"}}>
            <div style={{fontSize:"40px",marginBottom:"10px"}}>👥</div>
            <div style={{color:txt2,fontSize:"14px"}}>No users have signed up yet.</div>
            <div style={{color:txt2,fontSize:"12px",marginTop:"4px"}}>Share the URL — accounts appear here automatically.</div>
          </div>
        ):<>
          {/* ── Admins Section ──────────────────────── */}
          {adminUsers.length>0&&(
            <div style={{marginBottom:"6px"}}>
              <div style={{fontSize:"12px",fontWeight:600,color:acc,letterSpacing:"0.05em",textTransform:"uppercase",padding:"8px 4px"}}>
                👑 Admins ({adminUsers.length})
              </div>
              {adminUsers.map(analyst=>{
                const dayMarks = analyst.marks?.[adminDate] || {};
                const duration = calcAnalystDuration(dayMarks, true);
                const online = isAnalystOnline(analyst);
                const isAct = dayMarks.clock_in && !dayMarks.clock_out;
                const status = !dayMarks.clock_in ? "not-started" : dayMarks.clock_out ? "done"
                  : ((dayMarks.break_in_1&&!dayMarks.break_out_1)||(dayMarks.break_in_2&&!dayMarks.break_out_2)) ? "break"
                  : (dayMarks.lunch_in&&!dayMarks.lunch_out) ? "lunch" : "active";
                return (
                  <div key={analyst.id} style={{...s.card,borderLeft:`3px solid ${acc}`}}>
                    <div style={{display:"flex",alignItems:"center",gap:"10px",flexWrap:"wrap"}}>
                      <div style={{position:"relative",width:36,height:36}}>
                        <div style={{width:36,height:36,borderRadius:"50%",background:acc+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px"}}>👑</div>
                        <div style={{position:"absolute",bottom:-1,right:-1,width:12,height:12,borderRadius:"50%",background:online?"#10b981":"#ef4444",border:`2px solid ${card}`}} />
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                          <span style={{fontWeight:600,fontSize:"14px",color:txt}}>{analyst.fullName}</span>
                          <span style={{fontSize:"10px",color:online?"#10b981":"#ef4444",fontWeight:600}}>{online?"● Online":"● Offline"}</span>
                        </div>
                        {analyst.email&&<div style={{fontSize:"11px",color:txt2}}>{analyst.email}</div>}
                      </div>
                      {duration&&<span style={{background:isAct?acc+"22":"#d1fae5",color:isAct?acc:"#065f46",borderRadius:"999px",padding:"4px 10px",fontSize:"12px",fontWeight:700,fontVariantNumeric:"tabular-nums"}}>⏱ {duration}</span>}
                      <span style={{background:status==="active"?"#d1fae5":status==="done"?"#e0e7ff":"#f3f4f6",color:status==="active"?"#065f46":status==="done"?"#3730a3":"#6b7280",borderRadius:"999px",padding:"3px 10px",fontSize:"11px",fontWeight:600,textTransform:"capitalize"}}>{status==="not-started"?"—":status}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {/* ── Analysts Section ──────────────────── */}
          <div style={{fontSize:"12px",fontWeight:600,color:"#10b981",letterSpacing:"0.05em",textTransform:"uppercase",padding:"8px 4px"}}>
            📋 Analysts ({filteredAnalystUsers.length}{teamFilter!=="all"?` in ${teamFilter==="unassigned"?"Unassigned":getTeamName(teamFilter)}`:""})
          </div>
          {filteredAnalystUsers.length===0?(
            <div style={{...s.card,textAlign:"center",padding:"24px",color:txt2,fontSize:"13px"}}>{teamFilter!=="all"?"No analysts in this group.":"No analysts yet — they'll appear when they create accounts."}</div>
          ):filteredAnalystUsers.map(analyst=>{
            const dayMarks = analyst.marks?.[adminDate] || {};
            const duration = calcAnalystDuration(dayMarks, true);
            const isAct = dayMarks.clock_in && !dayMarks.clock_out;
            const online = isAnalystOnline(analyst);
              ...
              <div key={analyst.id} style={s.card}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"10px",flex:1,minWidth:"180px"}}>
                    <div style={{position:"relative",width:36,height:36}}>
                      ...
                      <div style={{position:"absolute",bottom:-1,right:-1,width:12,height:12,borderRadius:"50%",
                        background:online?"#10b981":"#ef4444",border:`2px solid ${card}`,
                        boxShadow:online?"0 0 6px #10b98166":"none"}} />
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                        <span style={{fontWeight:600,fontSize:"14px",color:txt}}>{analyst.fullName}</span>
                      ...
                      {analyst.email&&<div style={{display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap"}}>
                        <span style={{fontSize:"11px",color:txt2}}>{analyst.email}</span>
                        {analyst.teamId&&<span style={{fontSize:"10px",background:acc+"18",color:acc,borderRadius:"6px",padding:"1px 6px",fontWeight:600}}>🏷 {getTeamName(analyst.teamId)}</span>}
                      </div>}
                    </div>
                  </div>
                  <div style={{display:"flex",gap:"6px",alignItems:"center",flexWrap:"wrap"}}>
                    {duration&&<span style={{background:isAct?"linear-gradient(135deg,"+acc+","+acc+"cc)":status==="done"?"#d1fae5":"#e0e7ff",color:isAct?"#fff":status==="done"?"#065f46":"#3730a3",borderRadius:"999px",padding:"5px 12px",fontSize:"12px",fontWeight:700,fontVariantNumeric:"tabular-nums",boxShadow:isAct?"0 2px 8px "+acc+"44":"none",display:"inline-flex",alignItems:"center",gap:"5px"}}>
                      {isAct&&<span style={{display:"inline-block",width:6,height:6,borderRadius:"50%",background:"#fff",opacity:0.8,animation:"pulse 1.5s infinite"}}/>}⏱ {duration}
                    </span>}
                    <span style={{background:status==="active"?"#d1fae5":status==="break"?"#fef3c7":status==="lunch"?"#fce7f3":status==="done"?"#e0e7ff":"#f3f4f6",color:status==="active"?"#065f46":status==="break"?"#92400e":status==="lunch"?"#9d174d":status==="done"?"#3730a3":"#6b7280",borderRadius:"999px",padding:"3px 10px",fontSize:"11px",fontWeight:600,textTransform:"capitalize"}}>{status==="not-started"?"Not started":status}</span>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:"6px"}}>
                  {MARK_KEYS.map(({key,label,icon,color})=>{
                    ...
                    return (
                      <div key={key} style={{background:t?color+"12":(dm?"#3f3f46":"#f9fafb"),border:`1px solid ${t?color+"44":bdr}`,borderRadius:"8px",padding:"8px 10px",textAlign:"center"}}>
                        ...
                        <div style={{fontSize:"10px",color:t?txt:txt2,marginTop:"3px",fontVariantNumeric:"tabular-nums",fontWeight:t?600:400}}>{t?fmtMarkShort(t):"—"}</div>
                      </div>
                    );
                  })}
                </div>
        ...
        {/* ── Refresh & Reset (moved from Enterprise) ──── */}
        <div style={s.card}>
          ...
          <div style={{display:"flex",gap:"16px",flexWrap:"wrap",marginBottom:"14px"}}>
            <div style={{flex:1,minWidth:"180px"}}>
              ...
              <select value={refreshInterval} onChange={e=>setRefreshInterval(Number(e.target.value))} style={{...s.input,fontSize:"13px"}}>
            ...
            <div style={{flex:1,minWidth:"180px"}}>
              ...
              <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                <input type="time" value={resetTime} onChange={e=>setResetTime(e.target.value)} style={{...s.input,flex:1,fontSize:"13px"}} />
                {resetTime&&<button onClick={()=>setResetTime("")} style={{background:"none",border:`1px solid ${bdr}`,borderRadius:"8px",padding:"6px 10px",cursor:"pointer",fontSize:"12px",color:txt2}}>Clear</button>}
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap",alignItems:"center"}}>
            <button onClick={async()=>{
              setPopup({title:"🔄 Loading...",body:"Fetching data from database..."});
              await loadAnalystsFromDb();
              setTimeout(()=>{
                const ac=analysts.length;
                const mc=analysts.reduce((sum,a)=>sum+Object.keys(a.marks||{}).length,0);
                setPopup({title:`✅ Data refreshed`,body:`${ac} users loaded, ${mc} days with marks.`});
                setTimeout(()=>setPopup(null),5000);
              },500);
            }} style={s.btn(acc)}>🔄 Refresh data now</button>
            <button onClick={()=>{setEnterprise(prev=>({...prev,log:[]}));setPopup({title:"🔄 Log cleared",body:"Enterprise log reset."});setTimeout(()=>setPopup(null),3000);}} style={{...s.btn("#6b7280",true),fontSize:"12px"}}>Reset log</button>
            <span style={{fontSize:"11px",color:txt2}}>Loaded: {analysts.length} users · {activeAnalysts.length} active · {adminUsers.length} admins · {analystUsers.length} analysts</span>
          </div>
        </div>

        <div style={{...s.card,background:dm?"#1a1a2e":"#eef2ff",border:`1.5px solid ${acc}33`}}>
          <div style={{fontSize:"13px",color:txt2,lineHeight:1.7}}>
            <strong style={{color:txt}}>📡 Live tracking:</strong> Everyone who creates an account appears here automatically. Marks sync in real time. Use "Manage Team" to deactivate/reactivate members. Green dot = online, red = offline.
          </div>
        </div>
      </div>
      {BottomNav}
    ...
    <div style={s.wrap}>
      ...
      <div style={s.section}>
        ...
        <div style={s.card}>
          ...
          {/* Acting as — auto-linked for analysts, dropdown for admins */}
          {analysts.length>0&&(
            <div style={{marginBottom:"14px"}}>
              {isAdminView ? (
                <>
                  <label style={s.lbl}>Acting as (admin override)</label>
                  <select value={currentAnalystId} onChange={e=>setCurrentAnalystId(e.target.value)}
                    style={{...s.input,fontSize:"13px"}}>
                    <option value="">Admin (local only)</option>
                    {analysts.map(a=><option key={a.id} value={a.id}>{a.fullName}{a.enterpriseId?` (${a.enterpriseId})`:""}{a.email?` · ${a.email}`:""}</option>)}
                  </select>
                ...
                <>
                  {currentAnalystId ? (
                    <div style={{background:"#d1fae522",border:"1.5px solid #10b98144",borderRadius:lr,padding:"10px 14px",display:"flex",alignItems:"center",gap:"10px"}}>
                      <span style={{fontSize:"20px"}}>🟢</span>
                      <div>
                        <div style={{fontSize:"13px",fontWeight:600,color:"#10b981"}}>Auto-linked</div>
                        <div style={{fontSize:"12px",color:txt2}}>Signed in as <strong>{analysts.find(a=>a.id===currentAnalystId)?.fullName||"analyst"}</strong> — marks sync automatically</div>
                      </div>
                    </div>
                  ) : (
                    <div style={{background:"#fef3c7",border:"1.5px solid #f59e0b44",borderRadius:lr,padding:"10px 14px",display:"flex",alignItems:"center",gap:"10px"}}>
                      <span style={{fontSize:"20px"}}>⚠️</span>
                      <div>
                        <div style={{fontSize:"13px",fontWeight:600,color:"#92400e"}}>Not linked to roster</div>
                        <div style={{fontSize:"12px",color:"#92400e"}}>Ask your admin to add your email ({authUser?.email}) to the roster</div>
                      </div>
                    </div>
                  )}
                </>
