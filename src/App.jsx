...
export default function App({ authUser, profile, onLogout }) {
  ...
  const deactivateAnalyst = async (id) => {
    ...
    if (supabaseReady && supabase) {
      await supabase.from("analysts").update({ active: false }).eq("id", id);
      loadAnalystsFromDb();
    } else {
  ...
  const createTeam = async () => {
    ...
    if (supabaseReady && supabase) {
      await supabase.from("teams").insert({ name: newTeamName.trim(), created_by: authUser?.id });
      loadAnalystsFromDb();
    }
    setNewTeamName("");
  };
  ...
  const assignToTeam = async (analystId, teamId) => {
    if (supabaseReady && supabase) {
      await supabase.from("analysts").update({ team_id: teamId || null }).eq("id", analystId);
      loadAnalystsFromDb();
    }
  };
